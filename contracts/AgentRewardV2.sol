...
contract AgentRewardV2 is IAgentReward, Initializable, AccessControl, TokenSaver {
    ...
    mapping(address account => mapping(uint256 virtualId => Claim claim)) private _claimedStakerRewards;
    mapping(address account => mapping(uint256 virtualId => Claim claim)) private _claimedValidatorRewards;
    ...
    mapping(address validator => mapping(uint48 rewardId => uint256 amount)) private _validatorRewards;
    ...
    // Agent tokens before migration
    mapping(uint256 virtualId => address token) prevTokens;
    ...
    function getRewardSettings() public view returns (RewardSettingsCheckpoints.RewardSettings memory) {
    ...
    function getReward(uint256 virtualId, uint32 pos) public view returns (Reward memory) {
    ...
    function distributeRewardsForAgents(uint32 mainRewardIndex, uint256[] memory virtualIds) public onlyGov {
        RewardSettingsCheckpoints.RewardSettings memory settings = getRewardSettings();

        for (uint256 i = 0; i < virtualIds.length; i++) {
            _distributeAgentRewards(virtualIds[i], mainRewardIndex, settings);
        }
    }

    function _distributeProtocolRewards(uint256 amount) private returns (uint256) {
        RewardSettingsCheckpoints.RewardSettings memory rewardSettings = _rewardSettings.latest();
        uint256 protocolShares = (amount * rewardSettings.protocolShares) / DENOMINATOR;
    ...
    function _prepareAgentsRewards(
        ...
        _mainRewards.push(MainReward(SafeCast.toUint32(block.number), amount, agentCount, grandTotalStaked));
    ...
    function _distributeAgentRewards(///发钱给agent组织
        ...
        _distributeValidatorRewards(reward.validatorAmount, virtualId, reward.id, reward.totalStaked);
        _distributeContributorRewards(reward.contributorAmount, virtualId, settings);
    ...
    function _distributeContributorRewards(
        ...
        for (uint i = 0; i < services.length; i++) {
            ...
            impactAmount =
                ...
                _rewardImpacts[reward.id][serviceNftContract.getCore(serviceId)];
    ...
    function _getClaimableStakerRewardsAt(
        ...
        address delegatee = token.getPastDelegates(account, mainReward.blockNumber);

        if (delegatee == address(0)) {
            return 0;
        }

        RewardSettingsCheckpoints.RewardSettings memory settings = getPastRewardSettings(mainReward.blockNumber);

        uint256 validatorGroupRewards = _validatorRewards[delegatee][reward.id];

        uint256 tokens = token.getPastBalanceOf(account, mainReward.blockNumber);
        uint256 votes = IERC5805(stakingAddress).getPastVotes(delegatee, mainReward.blockNumber);

        return (((validatorGroupRewards * tokens) / votes) * uint256(settings.stakerShares)) / DENOMINATOR;
    }

    function _getClaimableStakerRewards(address staker, uint256 virtualId) internal view returns (uint256) {
        uint256 count = rewardCount(virtualId);
        if (count == 0) {
            return 0;
        }

        address stakingAddress = getVirtualTokenAddress(IAgentNft(agentNft), virtualId);

        Claim memory claim = _claimedStakerRewards[staker][virtualId];
        uint256 total = 0;
        for (uint256 i = claim.rewardCount; i < count; i++) {
            total += _getClaimableStakerRewardsAt(i, virtualId, staker, stakingAddress);
        }
    ...
    function _getClaimableValidatorRewardsAt(
        ...
        Reward memory reward = getReward(virtualId, SafeCast.toUint32(pos));
        MainReward memory mainReward = getMainReward(reward.mainIndex);
        RewardSettingsCheckpoints.RewardSettings memory rewardSettings = getPastRewardSettings(mainReward.blockNumber);

        uint256 validatorGroupRewards = _validatorRewards[validator][reward.id];

        return (validatorGroupRewards * (DENOMINATOR - uint256(rewardSettings.stakerShares))) / DENOMINATOR;
    }

    function _getClaimableValidatorRewards(address validator, uint256 virtualId) internal view returns (uint256) {
        uint256 count = rewardCount(virtualId);
        if (count == 0) {
            return 0;
        }
    ...
    function getChildrenRewards(uint256 nftId) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < children.length; i++) {
            ...
            childrenAmount += (childReward.parentAmount - childReward.totalClaimedParent);
        }
        return childrenAmount;
    }

    function _getClaimableServiceRewards(uint256 nftId) internal view returns (uint256 total) {
    ...
    // ----------------
    function _claimStakerRewards(address account, uint256 virtualId) internal noReentrant {
    ...
    function getServiceReward(uint256 nftId) public view returns (ServiceReward memory) {
        return _serviceRewards[nftId];
    }

    function _claimServiceRewards(uint256 nftId) internal {
        address account = _msgSender();
        require(IERC721(contributionNft).ownerOf(nftId) == account, "Not NFT owner");
        ...
        for (uint256 i = 0; i < children.length; i++) {
            ...
            childAmount = (childReward.parentAmount - childReward.totalClaimedParent);
    ...
    function claimAllRewards(uint256[] memory virtualIds, uint256[] memory contributionNftIds) public {
        address account = _msgSender();
        for (uint256 i = 0; i < virtualIds.length; i++) {
    ...
    function setRewardSettings(
        ...
        emit RewardSettingsUpdated(protocolShares_, contributorShares_, stakerShares_, parentShares_, stakeThreshold_);
    }
    ...
    function updateRefContracts(
        ...
        emit RefContractsUpdated(rewardToken_, agentNft_, contributionNft_, serviceNft_);
    }

    function setPrevTokens(uint256[] memory virtualIds, address[] memory tokens) external onlyGov {
        require(virtualIds.length == tokens.length, "Invalid input");
        for (uint256 i; i < virtualIds.length; i++) {
            prevTokens[virtualIds[i]] = tokens[i];
        }
    }

    function getVirtualTokenAddress(IAgentNft nft, uint256 virtualId) public view returns (address) {
