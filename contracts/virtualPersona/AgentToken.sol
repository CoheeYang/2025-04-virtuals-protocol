...
contract AgentToken is ContextUpgradeable, IAgentToken, Ownable2StepUpgradeable {
    ...
    function initialize(
        ...
        ERC20SupplyParameters memory supplyParams = abi.decode(supplyParams_, (ERC20SupplyParameters));
     ...
     */
    function _decodeBaseParams(address projectOwner_, bytes memory encodedBaseParams_) internal {
     ...
     */
    function _processSupplyParams(ERC20SupplyParameters memory erc20SupplyParameters_) internal {
        if (
     ...
     */
    function addLiquidityPool(address newLiquidityPool_) public onlyOwnerOrFactory {
     ...
     */
    function addValidCaller(bytes32 newValidCallerHash_) external onlyOwnerOrFactory {
     ...
     */
    function removeValidCaller(bytes32 removedValidCallerHash_) external onlyOwnerOrFactory {
        // Remove this from the enumerated list:
     ...
     */
    function setProjectTaxRecipient(address projectTaxRecipient_) external onlyOwnerOrFactory {
     ...
     */
    function setSwapThresholdBasisPoints(uint16 swapThresholdBasisPoints_) external onlyOwnerOrFactory {
