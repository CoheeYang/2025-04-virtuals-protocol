...
contract AgentFactoryV3 is IAgentFactoryV3, Initializable, AccessControl, PausableUpgradeable {
...
///主要逻辑函数，如果是bonding来的则会依赖bonding的tokenSupplyParams_，如果不是则会依赖此合约的param
    ...
    function _executeApplication(uint256 id, bool canStake, bytes memory tokenSupplyParams_) internal {
        ...
        address payable dao = payable(
            _createNewDAO(daoName, IVotes(veToken), application.daoVotingPeriod, application.daoThreshold)
        );
...
//setFunctions
    ...
    function setImplementations(address token, address veToken, address dao) public onlyRole(DEFAULT_ADMIN_ROLE) {
        tokenImplementation = token;
        daoImplementation = dao;
        veTokenImplementation = veToken;
    }

    function setMaturityDuration(uint256 newDuration) public onlyRole(DEFAULT_ADMIN_ROLE) {
