...
abstract contract ERC20Votes is ERC20VotesUpgradeable {
    ...
    function _getPastDelegates(address account, uint256 timepoint) internal view virtual returns (address) {
        uint48 currentTimepoint = clock();
        if (timepoint >= currentTimepoint) {
            revert ERC5805FutureLookup(timepoint, currentTimepoint);
        }
        return _delegateeCheckpoints[account].upperLookupRecent(SafeCast.toUint48(timepoint));
    }
}
